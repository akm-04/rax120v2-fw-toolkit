...
extern bd_accel_cb_t bd_accel_cb;
...
int ecm_nss_ipv4_http_accelerate_enable = 1;       /*DNI use to control accelerate HTTP packet or not*/
							...
							bool can_accel, bool is_routed, bool is_l2_encap, struct sk_buff *skb, uint16_t l2_encap_proto)
{
	...
	if (unlikely(!ct)) {
		...
		if (bd_accel_cb != NULL && !bd_accel_cb(ct)) {
			/*
...
int ecm_nss_ipv4_init(struct dentry *dentry)
{
	...
	/* DNI add: to control HTTP packet accelerate or not for block site or other func use.
	* Eg: Block site enable: set http_accelerate_enable to 0 , let HTTP packet pass up.
	* Eg: Block site disable: set http_accelerate_enable to 1, HTTP packets will be accelerated normally.
	*/
	if (!debugfs_create_u32("http_accelerate_enable", S_IRUGO | S_IWUSR, ecm_nss_ipv4_dentry,
				   (u32 *)&ecm_nss_ipv4_http_accelerate_enable)) {
	   DEBUG_ERROR("Failed to create ecm nss ipv4 http accelerate file in debugfs\n");
	   goto task_cleanup;
	}

...
/* The UDP port at our end */
    ...
    defined(CONFIG_TARGET_AM335X_EVM) || \
    defined(CONFIG_HW29766193P0P256P512P2X2P2X2)
...
#endif
#ifdef FIRMWARE_RECOVER_FROM_TFTP_SERVER
		if(len < tftp_block_size && NetRunTftpServer)
		{
...
static void tftp_timeout_handler(void)
{
	if(NetRunTftpServer && TftpServerWaiting){
		TftpLedCount++;
		if ((TftpLedCount % 2)== 1)
		{
			...
			eth_init();
#endif
			net_set_timeout_handler ((CONFIG_SYS_HZ*1)/4, tftp_timeout_handler);
		}else{
			/* power LED (GREEN) 0.75 second off */
			board_power_led(1);
			printf("            \b\b\b\b\b\b\b\b\b\b\b\b");
			net_set_timeout_handler ((CONFIG_SYS_HZ*3)/4, tftp_timeout_handler);
		}
	}else{

...
int get_eth_mac_address(uchar *enetaddr, uint no_of_macs)
{
	...
	if (sfi->flash_type != SMEM_BOOT_MMC_FLASH) {
		...
		ret = smem_getpart("ARTMTD", &start_blocks, &size_blocks);
#endif
    ...
    defined(CONFIG_HW29766193P0P256P512P2X2P2X2)
		...
		while (nand_block_isbad(&nand_info[flash_type], art_offset)){
			printf("Skipping bad block 0x%08llx\n", art_offset);
			art_offset += CONFIG_SYS_FLASH_SECTOR_SIZE;
		}
...
#ifdef CONFIG_HW29765960P0P4000P1000P4X4P4X4P4X4
		ret = get_partition_info_efi_by_name(blk_dev, "ARTMTD", &disk_info);
#endif
#if defined(CONFIG_HW29765589P0P512P1024P4X4P8X8) || \
    defined(CONFIG_HW29766094P0P256P512P2X2P2X2) || \
    defined(CONFIG_HW29766097P0P512P1024P2X2P2X2P2X2) || \

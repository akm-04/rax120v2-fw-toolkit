...
int checkboard(void)
{
    printf("U-boot dni1 V0.2 for DNI HW ID: 29766097; NAND flash 512MB; RAM 1024MB .\n");
	printf("developed based on 'qsdk-ipq807x.ilq.10.0-spf.10.0.FC' \n");
    return 0;
}
#endif
...
{	
	char runcmd[256];
	snprintf(runcmd, sizeof(runcmd), "nand erase 0xd00000 0x80000");
	run_command(runcmd, 0);
	return 0;
}
...
int do_dnibootm(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	snprintf(cmdbuf, sizeof(cmdbuf), "setenv fdt_high 0x4A400000");
	...
	snprintf(cmdbuf, sizeof(cmdbuf), "bootm 0x42000000#config@oak02");
	run_command(cmdbuf, 0);
	return 0;
}

...
version_string="$0 v0.2.0-ipq807x.ilq.10.0-spf.10.0"
...
(
    ...
    if [ "$bits" = "64" ]; then
        sed '/debug/d;/packages/d;/"ipq807x"/d;/t32/d;/ret_prep_32image/d;/Required/d;/nosmmu/d;/os.system(cmd)/d;/skales/d;/apps_64image/d;/os.chdir(ipq_dir)/d' -i update_common_info.py
        sed -i 's/.\/ipq/.\/ipq_x64/g' update_common_info.py
        sed -i 's/.\/ipq_x64_x64/.\/ipq_x64/g' update_common_info.py
    else
        sed '/debug/d;/packages/d;/"ipq807x_64"/d;/t32/d;/ret_prep_64image/d;/Required/d;/skales/d;/apps_32image/d;/nosmmu/d;/os.system(cmd)/d;/os.chdir(ipq_dir)/d' -i update_common_info.py
    fi

...
int misc_init_r(void)
{
	...
	gpio_tlmm_config(&gmac_gpio_config);
	gpio_set_value(LED_CLR_GPIO,0);
	if (ipq40xx_handle_gpio(GPIO_GET, LED_CLR_GPIO, 0))
		printf("RESET CLR TO 1\n");
	else
		printf("RESET CLR TO 0\n");

	gpio_set_value(LED_CLK_GPIO,0);
	if (ipq40xx_handle_gpio(GPIO_GET, LED_CLK_GPIO, 0))
		printf("RESET CLK TO 1\n");
	else
		printf("RESET CLK TO 0\n");
	led_off(0xffff);
	led_on(1 << WPS_LED);
...
int checkboard(void)
{
    printf("U-boot dni1 V0.3 for DNI HW ID: 29766094; NAND flash 256MB; RAM 512MB .\n");
...
/*return value 0: not pressed, 1: pressed*/
int board_wireless_button_is_press(void)
{
	if (ipq40xx_handle_gpio(GPIO_GET, WIRELESS_BUTTON, 0))
		return 0;
	else
		return 1;
}
...
int board_match_image_hw_id (ulong fw_image_addr)
{
	...
	/* Only check first 29 chars hw29766094p0p256p512p2x2p2x2 */
...
int do_button_test (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	if(!board_wireless_button_is_press())
		printf("WIRELESS button: NOT pressed\n");
	else
		printf("WIRELESS button: Pressed\n");
...
int do_upgrade_uboot (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	snprintf(runcmd, sizeof(runcmd), "setenv machid 801000f");
	run_command(runcmd, 0);
...
int do_dnibootm(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	snprintf(cmdbuf, sizeof(cmdbuf), "bootm 0x44000000#config@ac03");
	run_command(cmdbuf, 0);
	return 0;
}

...
#define pwr_led_off(led_mask)       do {                    \
               led_status &= ~(led_mask);              \
               flush_led_status(led_status, LED_NUM);  \
} while(0)

#define pwr_led_on(led_mask)      do {                    \
               led_status |= led_mask;                 \
               flush_led_status(led_status, LED_NUM);  \
} while(0)
/*
...
{
/*
	struct qca_gpio_config gmac_gpio_config = {2};
	...
	run_command(runcmd, 0);
	led_off(0xfff);
	led_on(1<<RING_LED_WHITE_SR);
	pwr_led_off(1 << POWER_LED_RED);
*/
	return 0;
}
...
int checkboard(void)
{
    printf("U-boot dni1 V0.2 for DNI HW ID: 29766193; NAND flash 256MB; RAM 512MB .\n");
	printf("developed based on 'qsdk-ipq6018.ilq.11.0FC' \n");
    return 0;
}
#endif
...
{
	if (ledstat == 0){
		pwr_led_on(1 << POWER_LED_RED);
		pwr_led_off(1 << POWER_LED_GREEN);
	}else
		pwr_led_off(1 << POWER_LED_RED);
}
...
void do_test_led(cmd_tbl_t *cmdtp, int flag, int argc,char * const argv[])
{
	...
	if (argc == 3){
		...
		strncpy(ledtype, argv[1],len);
		if (!strncmp(ledtype, "green", len))
			pin = POWER_LED_GREEN;		
		else if (!strncmp(ledtype, "red", len))
			pin = POWER_LED_RED;
		else if (!strncmp(ledtype, "blue_sr", len))
			pin = RING_LED_BLUE_SR;
		else if (!strncmp(ledtype, "red_sr", len))
			pin = RING_LED_RED_SR;
		else if (!strncmp(ledtype, "green_sr", len))
			pin = RING_LED_GREEN_SR;
		else if (!strncmp(ledtype, "white_sr", len))
			pin = RING_LED_WHITE_SR;
		else if (!strncmp(ledtype, "all", len))
			pin = 0;
		else
			goto error;

		strncpy(mode, argv[2],len);
		len = strlen(argv[2]);		
		if (len > 4)
			goto error;
		strncpy(mode, argv[2],len);
		if (!strncmp(mode, "on", len)){
			if(pin==POWER_LED_GREEN || pin==POWER_LED_RED)
				pwr_led_on(1<<pin);
			else if(pin == 0) {
...
U_BOOT_CMD(
	...
	"test_led <green/red/blue_sr/red_sr/green_sr/white_sr/all> <on/off>",
	"\n"
	"for example : test_led all on"
);
*/
...
int board_match_image_hw_id (ulong fw_image_addr)
{
	...
	/* Only check first 29 chars hw29765589p0p512p1024p4x4p8x8 */
...
{	
	char runcmd[256];
	snprintf(runcmd, sizeof(runcmd), "nand erase 0xd80000 0x80000");
	run_command(runcmd, 0);
	return 0;
}
...
int do_dnibootm(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	snprintf(cmdbuf, sizeof(cmdbuf), "setenv fdt_high 0x48500000");
	run_command(cmdbuf, 0);
	snprintf(cmdbuf, sizeof(cmdbuf), "setenv mtdids nand0=nand0;");
	run_command(cmdbuf, 0);
	snprintf(cmdbuf, sizeof(cmdbuf), "setenv bootargs;");
	run_command(cmdbuf, 0);
	...
	snprintf(cmdbuf, sizeof(cmdbuf), "bootm 0x40000000#config@cp03-c1");
	run_command(cmdbuf, 0);
	return 0;
}

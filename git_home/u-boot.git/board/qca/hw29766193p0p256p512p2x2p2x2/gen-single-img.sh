...
version_string="$0 v0.0.1-ipq6018.ilq.11.0-fc"
...
(
    cd "$work_dir"
    #rm -rf BOOT.AK.1.0 BOOT.BF.3.1.1 IPQ4019.ILQ.10.0 IPQ8064.ILQ.10.0 RPM.AK.* TZ.AK.* TZ.BF.2.7 WIGIG.TLN.1.0
    #cp -rf */* .
)
...
(
    ...
    if [ "$bits" = "64" ]; then
	sed '/debug/d;/packages/d;/"ipq6018"/d;/t32/d;/ret_prep_32image/d;/Required/d;/nosmmu/d;/os.system(cmd)/d;/skales/d;/os.chdir(ipq_dir)/d;/atfdir/d;/noac/d;/single-atf/d' -i update_common_info.py
	sed -i 's/.\/ipq/.\/ipq_x64/g' update_common_info.py
	sed -i 's/.\/ipq_x64_x64/.\/ipq_x64/g' update_common_info.py
    else
	sed '/debug/d;/packages/d;/"ipq6018_64"/d;/t32/d;/ret_prep_64image/d;/Required/d;/skales/d;/nosmmu/d;/os.system(cmd)/d;/os.chdir(ipq_dir)/d;/atfdir/d;/noac/d;/single-atf/d' -i update_common_info.py
    fi

    sed -i -e '/Generating 32 bit atf images/,+7d' update_common_info.py
    # RN Note:

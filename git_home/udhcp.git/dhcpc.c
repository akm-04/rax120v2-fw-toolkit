...
static u_int32_t old_requested_ip;
#endif
...
#endif
{
	...
	u_int32_t nak_server_addr;
	time_t t1 = 0, t2 = 0;
    u_int32_t xid = 0;
	time_t start = 0, lease;
	fd_set rfds;
	int retval;
	struct timeval tv;
	int c, len;
	...
	while (1) {
		...
		case 'A':
			...
			if (orange_flag == 1)
			{
				if (strstr(optarg, FTI_PREFIX) == NULL)
				{
					len = strlen(optarg) + strlen(FTI_PREFIX) + 11;
				}
				else
				{
				...
				if (strstr(optarg, FTI_PREFIX) == NULL) //search FTI_PREFIX
				{
					snprintf(client_config.authentication + OPT_DATA + 22, len + 1, "%s%s", FTI_PREFIX, optarg);
				}
				else
				{
	...
	if (orange_flag == 1)
	{
		char orange_vendor[32] = {0};
		char org_int_user_class[64] = {0};
		char org_iptv_user_class[64] = {0};
		
		system("/sbin/dhcp_config_action.sh >/dev/console");
		// set default values
		if (strcmp(cat_file(ENABLE_SPAIN_FILE), "1") == 0)
		{
			snprintf(orange_vendor, sizeof(orange_vendor), "%s", ORG_ES_OPT60);
			snprintf(org_int_user_class, sizeof(org_int_user_class), "%s", ORG_ES_INT_OPT77);
		}
		else
		{
			snprintf(orange_vendor, sizeof(orange_vendor), "%s", ORG_FR_OPT60);
			if (strcmp(cat_file(LB_VER_FILE), "4") == 0)
				snprintf(org_int_user_class, sizeof(org_int_user_class), "%s", ORG_FR_INT_LB4_OPT77);
			else
				snprintf(org_int_user_class, sizeof(org_int_user_class), "%s", ORG_FR_INT_LB3_OPT77);
		}
		if (strcmp(cat_file(ORANGE_IPTV_FILE), "1") == 0)
			snprintf(org_iptv_user_class, sizeof(org_iptv_user_class), "%s", ORG_ES_IPTV_OPT77);
		else
		{
			if (strcmp(cat_file(LB_VER_FILE), "4") == 0)
				snprintf(org_iptv_user_class, sizeof(org_iptv_user_class), "%s", ORG_FR_IPTV_LB4_OPT77);
			else
				snprintf(org_iptv_user_class, sizeof(org_iptv_user_class), "%s", ORG_FR_IPTV_LB3_OPT77);
		}

		// set option 60
		len = strlen(orange_vendor);
		if (client_config.vendor) free(client_config.vendor);
		client_config.vendor = xmalloc(len + 2);
		client_config.vendor[OPT_CODE] = DHCP_VENDOR;
		client_config.vendor[OPT_LEN] = len;
		client_config.vendor[OPT_DATA] = '\0';
		strncpy(client_config.vendor + OPT_DATA, orange_vendor, len);

		// set option 77
		if (strcmp(client_config.interface, "brwan") == 0)
			len = strlen(org_int_user_class);
		else if (strcmp(client_config.interface, "brotv") == 0)
			len = strlen(org_iptv_user_class);
		if (client_config.user_class) free(client_config.user_class);
		client_config.user_class = xmalloc(len + 2);
		client_config.user_class[OPT_CODE] = DHCP_USER_CLASS;
		client_config.user_class[OPT_LEN] = len+1;
		client_config.user_class[OPT_DATA] = len;
		if (strcmp(client_config.interface, "brwan") == 0)
			strncpy(client_config.user_class + OPT_DATA + 1, org_int_user_class, len);
		else if (strcmp(client_config.interface, "brotv") == 0)
			strncpy(client_config.user_class + OPT_DATA + 1, org_iptv_user_class, len);		
	}
#endif

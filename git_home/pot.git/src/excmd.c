...
int ntpst_func(int argc, char *argv[])
{
	...
	/* T param get from config as following code ,so ellipsis*/
#else
	...
	if (0 == sign) {
		if (!memcmp(&tmp, ntptime, sizeof(tmp))) {
...
int stamac_func(int argc, char *argv[])
{
	...
	unsigned int gmac[6]={0};
	char mtd[32] = {0}, tmp_mac[32] = {0};
	
	unsigned char mac_emmc[6]={0};
	...
	if (argc >= 2 && !strcmp(argv[1], "get"))
		...
		sign = 1;
		if (argv[2] != NULL)
			strncpy(tmp_mac, argv[2], sizeof(tmp_mac)-1);
		else
			stamac_usage(argv[0]);
	}
	else
...
int netconn_func_for_nand(int argc, char *argv[]){
	...
	if (0 == sign) {
		...
		skip_size = find_good_nand_block(devfd, FIRST_NTP_TIME_OFFSET);   
		if(skip_size == -1){
			fprintf(stderr, "netconn-stamac: no good block, exit now\n");	
			exit(1);
		}
	        lseek(devfd, skip_size, SEEK_SET);
		...
		snprintf(flash_erase_cmd, sizeof(flash_erase_cmd), "flash_erase %s 0x0040000 1", pot_mtd_dev);
		system(str_replace_n(flash_erase_cmd));

		if (0xFFFFFFFF != *ntptime) {
			skip_size = find_good_nand_block(devfd, FIRST_NTP_TIME_OFFSET);   
			if(skip_size == -1){
				fprintf(stderr, "netconn-ntptime: no good block, exit now\n");	
				exit(1);
			}
...
int netconn_func_for_emmc(int argc, char *argv[]){
	...
	int count = 0;
	unsigned char current_con[2]={0}, res_con[2]={0};

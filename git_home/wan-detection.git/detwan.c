...
static int verify_packet(int fd, uint32 xid, uint8 *arp, int *bpain)
{
	...
	if (!UDP_IP(&packet.ip) ||packet.udp.uh_dport != htons(CLIENT_PORT))
		return 0;
...
/*======================================================
                      [ L2TP Detection ]
======================================================*/
static uint16 payload_len, l2tp_cursor, alloc_len;
...
int l2tp_add_avp(struct l2tp_dgram *dgram, int mandatory, uint16 len, uint16 type, void *val)
{
	...
	if ((l2tp_cursor + len) > alloc_len){
		return -1;
	}

	dgram->data[l2tp_cursor] = (mandatory ? AVP_MANDATORY_BIT : 0); /* no hidden */
	dgram->data[l2tp_cursor] |= (len >> 8);
	dgram->data[l2tp_cursor+1] = (len & 0xFF);
	dgram->data[l2tp_cursor+2] = 0;
	dgram->data[l2tp_cursor+3] = 0; /* vendor is always 'VENDOR_IETF == 0' */
	dgram->data[l2tp_cursor+4] = (type >> 8);
	dgram->data[l2tp_cursor+5] = (type & 0xFF);

	if (len > 6){
		avp_len = len - 6;
		memcpy(dgram->data + l2tp_cursor + 6, val, avp_len);
	}
	l2tp_cursor += len;
	payload_len = l2tp_cursor;
...
static void l2tp_dgram_init(struct l2tp_dgram *dgram, uint16 msg_type)
{
	uint16 u16_val;
	uint32 u32_val;
	uint16 tid, val;
...
static int tunnel_send_SCCRQ(int fd, struct l2tp_dgram *dgram)
{
	socklen_t slen;
	uint8 *len_ptr = NULL, buf[MAX_PACKET_LEN]={0};
	int cursor = 2;
	size_t total_len;
	...
	memcpy(buf + 12, dgram->data, payload_len);

	slen = sizeof(to);
	memset(&to, 0, sizeof(struct sockaddr_in));
	to.sin_family = AF_INET;
	to.sin_port = htons(L2TP_PORT);
	to.sin_addr.s_addr = inet_addr(DEF_L2TP_SERVER);

	if (!ifconfig(wan_if_name, IF_NONE, DEF_L2TP_IPADDR, DEF_L2TP_NETMASK)){
		return 0;
	}
	sprintf(cmd, "/sbin/route add -net %s netmask 255.255.255.255 %s", DEF_L2TP_SERVER, wan_if_name);
	system(cmd);
	return sendto(fd, buf, total_len, 0, (struct sockaddr *)&to, slen);
}
...
static int l2tp_wait_packet(int fd)
{
	...
	for (;;) {
		...
		from.sin_addr.s_addr = inet_addr(DEF_L2TP_SERVER);
		slen = sizeof(from);
...
static int L2TP_Detection(void)
{
	...
	/* Set IPtables Rules */
	sprintf(cmd, "/usr/sbin/iptables -t mangle -I PREROUTING -i %s -s %s -d %s -p udp --sport %d -j ACCEPT", wan_if_name, DEF_L2TP_SERVER, DEF_L2TP_IPADDR, L2TP_PORT);
	system(cmd);
	...
	for (i = 0; i < 3; i++){
		l2tp_dgram_init(&dgram, MESSAGE_SCCRQ);
		sret = tunnel_send_SCCRQ(fd, &dgram);
		if( sret < 0){
			system(cmd);
			continue;
		}
...
static int checkType(uint32_t ConnType)
{
	...
	if ( ConnType & L2TPFound) {
		wanmode = STATUS_L2TP;
	} else if ( ConnType & PPPoEFound) {
		wanmode = STATUS_PPPOE;
	} else if ( ConnType & PPTPFound) {
		wanmode = STATUS_PPTP;
	} else {

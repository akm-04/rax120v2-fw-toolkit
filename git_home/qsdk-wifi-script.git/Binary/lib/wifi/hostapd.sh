...
hostapd_set_bss_options() {
	...
	[ -n "$wps_possible" -a -n "$config_methods" ] && {
		...
		config_get device_name "$vif" wps_device_name "RAX120"
		config_get manufacturer "$vif" wps_manufacturer "NTGR"
		config_get model_name "$vif" model_name "RAX120"
		config_get model_number "$vif" model_number "RAX120"
	...
	then
		...
		if [ -z "$ieee80211w" ]; then
			if [ "$wfa_require_conf" = "1" ]; then
				...
				# Default value in QSDK SPF 8.0 CSU1
				ieee80211w=0
			fi
		fi
	...
	if [ ! -n "$bss_load_update_period" ]; then
		if [ "$sae" = "1" ] && \
		   [ -n "$bss_load_update_period_sae_default" ]; then
			bss_load_update_period=$bss_load_update_period_sae_default
		fi
	fi
...
$hostapd_cfg
EOF
	if [ -z "${no_nconfig}" ] || [ "$driver" != "nl80211" ]; then
		qca_hostapd_fixup_hw_mode hwmode
...
#
# Fix up value of input variable to be one of values below which can be
# accepted by "hw_mode" option of QCA hostapd:
#
#     a, b, g, ad, any
#
# $1: Name of variable whose value should be fixed up for "hw_mode"
#
qca_hostapd_fixup_hw_mode() {
	local hw_mode_var=$1
	local hw_mode_value
	eval "hw_mode_value=\$$hw_mode_var"

	# Temporarily follow QCA to not use "b" and "ad" here.
	case "$hw_mode_value" in
	*axg|*ng|*g|*b) hw_mode_value=g;;
	*axa|*na|*a|*ac) hw_mode_value=a;;
	esac

	eval "$hw_mode_var=$hw_mode_value"
}

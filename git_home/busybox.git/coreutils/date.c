...
static int get_time_zone(char *buf)
{
#define TIME_ZONE_FILE "/tmp/time_zone"
	...
	if (!(fp = fopen(TIME_ZONE_FILE, "r"))) return -1;
...
int date_main(int argc UNUSED_PARAM, char **argv)
{
	...
	if (*argv)
	 ...
	 if (opt & OPT_KERNELTZ) {
		 const struct timeval *tvnull = NULL;
		 tt = time(NULL);
		 localtime_r(&tt, &tm_time);

		 /* workaround warp_clock() on first invocation */
		 memset(&tz, 0, sizeof(tz));
		 settimeofday(tvnull, &tz);
	...
	if (get_time_zone(time_zone) == 0 && (*time_zone != '\0') ) {

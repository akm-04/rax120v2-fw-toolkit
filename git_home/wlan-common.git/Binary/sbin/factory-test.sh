...
#
# "auto calibration": automatically execute preparation commands for WiFi
# calibration (cal) and/or RF test mode (ota).
#
if [ "$1" = "mode" ]; then
    #
    # Has been calibrated?
    #
    # 0: no, (other values): yes
    #
    cal="$(config get cal)"

    #
    # Has been tested in RF test mode?
    #
    # 0: no, (other values): yes
    #
    ota="$(config get ota)"
    ...
    if [ "$ota" = "0" ]; then
        ...
        killall ftm
        /etc/init.d/ftm stop
        killall diag_socket_app

        factory_serverip="$(config get factory_serverip)"
        factory_serverip="${factory_serverip:-192.168.1.10}"
        ...
        insmod wifi_3_0
        insmod wifi_2_0
        diag_socket_app -a "$factory_serverip" &
        /etc/init.d/ftm start
        ftm -n &
    fi

    ledcontrol -n power -c white -s on

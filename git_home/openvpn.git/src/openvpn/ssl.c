            ...
            interval_t *wakeup)
{
    ...
    struct key_state *ks = &session->key[KS_PRIMARY];      /* primary key */
    struct key_state *ks_lame = &session->key[KS_LAME_DUCK]; /* retiring key */
    char sysmsg[31] = {'\0'}, *start = NULL, *c_addr = NULL;
...
error:
    ...
    if(start = msg_get_prefix()){
	snprintf(sysmsg, sizeof(sysmsg) ,"%s", start);
	/*max sysmsg="client/192.168.100.100:12345," */
	if((start = c_addr = strchr(sysmsg,'/')) && strtok(start, ":")){
		syslog(4, "[OpenVPN, connection fail] from remote IP address: %s", ++c_addr);
	}
	/*This 'else' condition  should be the normal case because "connection fail" means it's not client yet,
	 * so without substring "client/". In case of any issue, still keep the judgement in 'if' condition
	 */
	else if((start = sysmsg) && strtok(start, ":"))

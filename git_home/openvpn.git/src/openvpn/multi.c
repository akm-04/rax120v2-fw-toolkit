                 ...
                 const unsigned int flags)
{
        ...
        && !mroute_addr_equal(addr, &m->local))
    {
		...
		record_incoming_tap_mac(mroute_addr_print(&newroute->addr, &gc));
...
void record_incoming_tap_mac(char *mac)
{
	char buf[24] = {0};
	FILE *fp = NULL;
	...
	fp = fopen(OPENVPN_TAP_DEVICE_RECORD, "a+");
	if (fp != NULL)
	{
		while (fgets(buf, sizeof(buf), fp) != NULL)
		{
			if (strncmp(mac, buf, strlen(mac)) == 0)
			{
				fclose(fp);
				return;
			}

...
if [ "x$(/bin/config get agree_collect_TC)" = "x0" ];then
	echo "AnalyticOptinOptout=false" > /tmp/AnalyticOptinOptout_status
else
	echo "AnalyticOptinOptout=true" > /tmp/AnalyticOptinOptout_status
fi
...
if [ "`/bin/config get dgc_func_have_armor`" = "1" ];then
	tar -zcvf armor.tar.gz /tmp/upagent.log /tmp/UpAgent.log /tmp/dal_ash.log /tmp/bitdefender_logs.tar.gz /tmp/dal_ash_status.log
	armor_log="/tmp/armor.tar.gz"
fi

zip -j debug-log.zip NETGEAR_$module_name.cfg $logs_to_be_saved /tmp/dal/d2d/* /tmp/dal_ash.log /tmp/dalh.log /tmp/upagent.log /tmp/UpAgent.log /tmp/fing_dil.log /tmp/dillog /tmp/xagent.log /tmp/bst.log $armor_log /var/log/netscan/* /tmp/AnalyticOptinOptout_status

...
lbd_updateDNI_config () {
    ...
    local lbd_config
    local lbd_enable
    local lbd_InactDetectionFromTx
    ...
    lbd_InactDetectionFromTx=$(config get lbd_InactDetectionFromTx)
    if [ -n "$lbd_InactDetectionFromTx" ]; then
        uci set lbd.config.InactDetectionFromTx=$lbd_InactDetectionFromTx
    else
        config set lbd_InactDetectionFromTx=$(uci get lbd.lbd.config.InactDetectionFromTx)
    fi
    ...
    if [ -n "$lbd_StartInBTMActiveState" ]; then

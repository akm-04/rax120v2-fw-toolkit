...
enable_bond=0
BondEth=bond0
# BondEth2(bond2) is only for RAX120 and BondEth(bond0) is not used
BondEth2=bond2
...
eth_mask()
{
	...
	2) echo 2;;
	3) echo 1;;
	5) echo 16;;  # For RAX120 5G Ethernet port --- eth5
	esac
}
...
sw_configvlan_iptv() # $1: iptv_mask
{
	...
	if [ $(( 48 & $mask )) -eq 0 ]; then
		brctl addif br0 $RawEthLanX
	else
		brctl addif brwan $RawEthLanX
	fi
}

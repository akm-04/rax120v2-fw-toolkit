...
thank_login_info="Thank you for purchasing Netgear's RAX120 Wireless-N Router. Be sure to use the Firmware Upgrade periodically to take advantage of improvements and feature additions NETGEAR offers for this product.<BR><BR>To take advantage of the Wireless-N feature offered by the router, use a wireless adapter that displays this logo."
thank_login_info_RangeMax="Thank you for purchasing Netgear's RAX120 RangeMax (TM) Wireless-N 600 Router. Be sure to use the Firmware Upgrade periodically to take advantage of improvements and feature additions NETGEAR offers for this product.<BR><BR>To take advantage of the Wireless-N feature offered by the router, use a wireless adapter that displays this logo."
...
error_module="Error upgrade file module name! (Module name is \"RAX120\") "
error_language_form="Incorrect upgrade file language name. (The language name is\"RAX120-*-Language-table\".)"
...
h_qos_fast="<body bgcolor=#0099cc><P><font size=4><B>QoS Settings Help</B></font></P><p>QoS is an advanced feature that you can use to prioritize some Internet applications and online gaming, and to minimize the impact when the bandwidth is busy. <hr><p><a name=wifi></a><b>Wi-Fi Multimedia (WMM) Settings</b><P>WMM (Wireless Multimedia) is a subset of the 802.11e standard. WMM allows wireless traffic to have a range of priorities, depending on the kind of data. Time-dependent information, like video or audio, has a higher priority than normal traffic. For WMM to function correctly, wireless clients must also support WMM.<hr><p><a name=qos_on></a><b>Turn Internet Access QoS On</b><p> If this feature is enabled, the QoS function prioritizes Internet traffic. For applications, online gaming, an Ethernet LAN port, or a specified MAC address that already exists in the drop-down list, you can modify the priority level by clicking the <b>Edit</b> button. You can click the <b>Delete</b> button to erase the priority rule. You can also define the priority policy for each online game, application, LAN port, or the computer's MAC address by clicking the <b>Add Priority Rule</b> button.</P><HR><A name=Bandwidth></A><p><B>Bandwidth Control</B></p><p>To setup the total maximum uplink bandwidth by <B>Turn Bandwidth Control On</B>.And you can click the <B>check</B> button to detect current uplink bandwidth which will help you to determinate the maximum bandwidth setting. </p><p><b>To save or cancel changes:</b><p>Click <b>Apply</b> to have your changes take effect. <p>Click <b>Cancel</b> to return to the previous settings. <hr><a name=FastTrack></a><p><B>FastTrack </B> The first time you turn on FastTrack, you need to specify the bandwidth that you want to reserve, and set up the push button for FastTrack. Fill in all the fields on this screen, and click <B>Apply</B> so your changes take effect.<B>FastTrack  Status:</B> This field shows whether FastTrack is currently turned on or off.<B>Turn on FastTrack: </B> By default, FastTrack is turned off, and the push button on the front or rear panel works as a Wi-Fi Protected Setup (Wi-Fi Protected Setup) button. (If your wireless router does not have a physical Wi-Fi Protected Setup button, go to the FastTrack tab in the GUI to configure it.) To turn on FastTrack, select this check box. <B>Bandwidth Reserved for Trusted IP Address</B> Specify what percentage of the bandwidth on the wireless router to reserve when you connect from a trusted IP address. This prevents others from using up your bandwidth. When you connect to the router from the trusted IP address, you then have guaranteed bandwidth for video streaming applications. There are 4 options: 50%, 60%, 70%, and 80%.<B>Push Button:</B> You can set up the push button on the front panel to work in two different ways: Wi-Fi Protected Setup (default) or FastTrack. Select the radio button for the option that you want.</p></body>"
wep_or_wps="WEP security with shared key authentication does not work with Wi-Fi Protected Setup. Wi-Fi Protected Setup will be inaccessible. Do you want to continue?"
wlan_adv_wps="Wi-Fi Protected Setup Settings"
wlan_adv_wps_pin3="In auto disabled mode, router's Wi-Fi Protected Setup LED will keep blinking slowly"
h_wladv_3rest="<HR><A name=wps></A><P><B>Wi-Fi Protected Setup Settings</B></P><p>Router's PIN</p><p>This is the PIN number you use on a registrar (e.g., from Network Explorer on a Vista Windows PC) to configure the router's wireless settings through Wi-Fi Protected Setup. You can also find the PIN on the router's product label. </p><P>Disable Router's PIN <P>You can configure the router's wireless settings or add a wireless client through Wi-Fi Protected Setup using the router's PIN only when the PIN is enabled. The router's PIN can be disabled temporarily when the router detects suspicious attempts to break into the router's wireless settings by using the router's PIN through Wi-Fi Protected Setup.You can manually enable this function by clearing the <b></b> check box and clicking the <b>Apply</b> button. </P><P><b>Keep Existing Wireless Settings</b> <P>This shows whether the router is in the Wi-Fi Protected Setup configured state.<br>If this option is not selected, adding a new wireless client will change the router's wireless settings to an automatically generated random SSID and security key. In addition, if this option is selected, some external registrars (e.g., Network Explorer on Vista Windows) might not see the router.<br>Configuring basic wireless settings from the router's management GUI selects this option automatically.</P><HR><A name=access></A><P><B>Wireless Card Access List</B></P><P>By default, any wireless PC that is configured with the correct SSID will be allowed access to your wireless network. For increased security, you can restrict access to the wireless network to allow only specific PCs based on their MAC addresses. On the Wireless Settings screen, click the <b>Setup Access List</b> button to display the Wireless Access List screen.</P><P><B>To Save or Cancel Changes</B></P><P>Click <B>Apply</B> to have your changes take effect. <BR>Click <B>Cancel</B> to return to the previous settings. </P></body>"
h_wladv_3rest_pin="<HR><A name=wps></A><P><B>Wi-Fi Protected Setup Settings</B></P><p>Router's PIN</p><p>This is the PIN number you use on a registrar (e.g., from Network Explorer on Windows 7) to configure the router's wireless settings through Wi-Fi Protected Setup. Usually you can also find the PIN on the router's product label.</p><p>Enable Router's PIN</p><P>You can configure the router's wireless settings or add a wireless client through Wi-Fi Protected Setup using the router's PIN only when the PIN is enabled.</p><p>Brute Force Attack Protection</p><p>The Wi-Fi Protected Setup router's PIN is susceptible to a brute force attack. Enabling this protection option can disable the router's PIN automatically when the router detects a suspicious brute force attack. Once the router's PIN is disabled automatically, it remains disabled until the router reboots or you can manually enable the router's PIN by checking the Enable Router's PIN check box and clicking the <b>Apply</b> button.<br>When the protection is enabled, you can configure the number of accumulated failed attempts that is allowed before the router's PIN is disabled automatically.</p><P>Keep Existing Wireless Settings<P>This shows whether the router is in the Wi-Fi Protected Setup configured state.<br>If this option is not selected, adding a new wireless client will change the router's wireless settings to an automatically generated random SSID and security key. In addition, if this option is selected, some external registrars (e.g., Network Explorer on Windows 7) might not see the router.<br>Configuring basic wireless settings from the router's management GUI selects this option automatically.</P><HR><A name=access></A><P><B>Wireless Card Access List</B></P><P>By default, any wireless PC that is configured with the correct SSID will be allowed access to your wireless network. For increased security, you can restrict access to the wireless network to allow only specific PCs based on their MAC addresses. On the Wireless Settings screen, click the <b>Setup Access List</b> button to display the Wireless Access List screen.</P><P><B>To Save or Cancel Changes</B></P><P>Click <B>Apply</B> to have your changes take effect. <BR>Click <B>Cancel</B> to return to the previous settings. </P></body>"
h_wladv_3="<P>Fragmentation Threshold, CTS/RTS Threshold, Preamble Mode<P>These settings are reserved for wireless testing and advanced configuration only. Do not change these settings.</P><HR><A name=wps></A><P><B>Wi-Fi Protected Setup Settings</B></P><p>Router's PIN</p><p>This is the PIN number you use on a registrar (e.g., from Network Explorer on a Vista Windows PC) to configure the router's wireless settings through Wi-Fi Protected Setup. You can also find the PIN on the router's product label. </p><P>Disable Router's PIN <P>You can configure the router's wireless settings or add a wireless client through Wi-Fi Protected Setup using the router's PIN only when the PIN is enabled. The router's PIN can be disabled temporarily when the router detects suspicious attempts to break into the router's wireless settings by using the router's PIN through Wi-Fi Protected Setup.You can manually enable this function by clearing the <b></b> check box and clicking the <b>Apply</b> button. </P><P><b>Keep Existing Wireless Settings</b> <P>This shows whether the router is in the Wi-Fi Protected Setup configured state.<br>If this option is not selected, adding a new wireless client will change the router's wireless settings to an automatically generated random SSID and security key. In addition, if this option is selected, some external registrars (e.g., Network Explorer on Vista Windows) might not see the router.<br>Configuring basic wireless settings from the router's management GUI selects this option automatically.</P><HR><A name=access></A><P><B>Wireless Card Access List</B></P><P>By default, any wireless PC that is configured with the correct SSID will be allowed access to your wireless network. For increased security, you can restrict access to the wireless network to allow only specific PCs based on their MAC addresses. On the Wireless Settings screen, click the <b>Setup Access List</b> button to display the Wireless Access List screen.</P><P><B>To Save or Cancel Changes</B></P><P>Click <B>Apply</B> to have your changes take effect. <BR>Click <B>Cancel</B> to return to the previous settings. </P></body>"
h_wladv_3_pin="<P>Fragmentation Threshold, CTS/RTS Threshold, Preamble Mode<P>These settings are reserved for wireless testing and advanced configuration only. Do not change these settings.</P><HR><A name=wps></A><P><B>Wi-Fi Protected Setup Settings</B></P><p>Router's PIN</p><p>This is the PIN number you use on a registrar (e.g., from Network Explorer on Windows 7) to configure the router's wireless settings through Wi-Fi Protected Setup. Usually you can also find the PIN on the router's product label.</p><p>Enable Router's PIN</p><P>You can configure the router's wireless settings or add a wireless client through Wi-Fi Protected Setup using the router's PIN only when the PIN is enabled.</p><p>Brute Force Attack Protection</p><p>The Wi-Fi Protected Setup router's PIN is susceptible to a brute force attack. Enabling this protection option can disable the router's PIN automatically when the router detects a suspicious brute force attack. Once the router's PIN is disabled automatically, it remains disabled until the router reboots or you can manually enable the router's PIN by checking the Enable Router's PIN check box and clicking the <b>Apply</b> button.<br>When the protection is enabled, you can configure the number of accumulated failed attempts that is allowed before the router's PIN is disabled automatically.</p><P>Keep Existing Wireless Settings<P>This shows whether the router is in the Wi-Fi Protected Setup configured state.<br>If this option is not selected, adding a new wireless client will change the router's wireless settings to an automatically generated random SSID and security key. In addition, if this option is selected, some external registrars (e.g., Network Explorer on Windows 7) might not see the router.<br>Configuring basic wireless settings from the router's management GUI selects this option automatically.</P><HR><A name=access></A><P><B>Wireless Card Access List</B></P><P>By default, any wireless PC that is configured with the correct SSID will be allowed access to your wireless network. For increased security, you can restrict access to the wireless network to allow only specific PCs based on their MAC addresses. On the Wireless Settings screen, click the <b>Setup Access List</b> button to display the Wireless Access List screen.</P><P><B>To Save or Cancel Changes</B></P><P>Click <B>Apply</B> to have your changes take effect. <BR>Click <B>Cancel</B> to return to the previous settings. </P></body>"
h_wladv_wps_1="<HR><A name=wps></A><P><B>Wi-Fi Protected Setup Settings</B></P><p>Router's PIN</p><p>This is the PIN number you use on a registrar (e.g., from Network Explorer on Windows 7) to configure the router's wireless settings through Wi-Fi Protected Setup. Usually you can also find the PIN on the router's product label.</p><p>Enable Router's PIN</p><P>You can configure the router's wireless settings or add a wireless client through Wi-Fi Protected Setup using the router's PIN only when the PIN is enabled.</p><p>Brute Force Attack Protection</p><p>The Wi-Fi Protected Setup router's PIN is susceptible to a brute force attack. Enabling this protection option can disable the router's PIN automatically when the router detects a suspicious brute force attack. Once the router's PIN is disabled automatically, it remains disabled until the router reboots or you can manually enable the router's PIN by checking the Enable Router's PIN check box and clicking the <b>Apply</b> button.<br>When the protection is enabled, you can configure the number of accumulated failed attempts that is allowed before the router's PIN is disabled automatically.</p><P>Keep Existing Wireless Settings<P>This shows whether the router is in the Wi-Fi Protected Setup configured state.<br>If this option is not selected, adding a new wireless client will change the router's wireless settings to an automatically generated random SSID and security key. In addition, if this option is selected, some external registrars (e.g., Network Explorer on Windows 7) might not see the router.<br>Configuring basic wireless settings from the router's management GUI selects this option automatically.</P>"
h_wladv_wps_2="<HR><A name=wps></A><P><B>Wi-Fi Protected Setup Settings</B></P><p>Router's PIN</p><p>This is the PIN number you use on a registrar (e.g., from Network Explorer on a Vista Windows PC) to configure the router's wireless settings through Wi-Fi Protected Setup. You can also find the PIN on the router's product label. </p><P>Disable Router's PIN<P>You can configure the router's wireless settings or add a wireless client through Wi-Fi Protected Setup using the router's PIN only when the PIN is enabled. The router's PIN can be disabled temporarily when the router detects suspicious attempts to break into the router's wireless settings by using the router's PIN through Wi-Fi Protected Setup.You can manually enable this function by clearing the <b></b> check box and clicking the <b>Apply</b> button. </P><P><b>Keep Existing Wireless Settings</b> <P>This shows whether the router is in the Wi-Fi Protected Setup configured state.<br>If this option is not selected, adding a new wireless client will change the router's wireless settings to an automatically generated random SSID and security key. In addition, if this option is selected, some external registrars (e.g., Network Explorer on Vista Windows) might not see the router.<br>Configuring basic wireless settings from the router's management GUI selects this option automatically.</P>"
wpae_or_wps="WPA/WPA2 Enterprise cannot work with Wi-Fi Protected Setup. Wi-Fi Protected Setup is going to become inaccessible. Do you want to continue?"
wps_cancelByuser="Wi-Fi Protected Setup procedure has been canceled by user."
add_wps_client_header="Add Wi-Fi Protected Setup Client"
add_wps_client_p="New and easy way to connect to the wireless router using Wi-Fi Protected Setup (Wi-Fi Protected Setup)"
add_wps_client_text1="A wireless client has to support the Wi-Fi Protected Setup function if you want to use this wizard to add the client to your Wi-Fi Protected Setup-enabled wireless router."
add_wps_client_text2="Please check the user manual and gift box of your wireless client to see whether it supports the Wi-Fi Protected Setup function."
add_wps_client_text3="If your wireless client does not support the Wi-Fi Protected Setup function, you have to configure your wireless client manually so that it has the same SSID and wireless security settings as this router."
wps_pin_text1="This is the security PIN of the Wi-Fi Protected Setup client. While connecting, Wi-Fi Protected Setup-enabled adapters provide a randomly-generated security PIN."
wps_connect_status_text="Please click the software or hardware button on the client to start the Wi-Fi Protected Setup process..."
wps_connect_status_text1="Please click the software button on the client to start the Wi-Fi Protected Setup process..."
wps_add_failure_text1="Wi-Fi Protected Setup procedure has timed out."
wps_add_failure_chk=" Please check whether the client supports the Wi-Fi Protected Setup function."
h_wps="<body bgcolor=#0099cc ><P><font size=4><B>Add Wi-Fi Protected Setup Client Help</B></font></P><p><a name=internet></a><a name=meter></a>WPS stands for Wi-Fi Protected Setup. This is a process that lets you add a wireless client to the network easily, without the need to specifically configure the wireless settings such as SSID, security mode, and passphrase.<p>You can select to add a wireless client using either the Push Button or PIN Number method.<p>With the Push Button method, you complete the process by clicking a button on the client (either a physical hardware button or a software button on the client's utility GUI).<p>With the PIN Number method, you have to enter the client's PIN here. You have to start the client Wi-Fi Protected Setup process at the same time. You can find the client's PIN on the client's utility GUI.</p></body>"
wps_wzd_head="Wi-Fi Protected Setup Wizard"
wps_warning1="Wi-Fi Protected Setup requires SSID broadcasting in order to work. If you make this change, Wi-Fi Protected Setup will become inaccessible. Do you want to continue?"
wps_warning2="WEP or WPA-Personal [TKIP] security authentication cannot work with Wi-Fi Protected Setup. Wi-Fi Protected Setup is going to become inaccessible. Do you want to continue?"
...
wds_or_wps="Wireless Repeating Function cannot work with Wi-Fi Protected Setup, Wi-Fi Protected Setup is going to become inaccessible. Do you want to continue?"
guest_tkip_300_150="WPA-Personal [TKIP] in Guest Network ONLY operates at \"Up to 54Mbps\" (legacy G) mode, not N mode."
guest_tkip_aes_300_150="NETGEAR recommends that you use WPA2-Personal [AES] in Guest Network to get full N rate support."

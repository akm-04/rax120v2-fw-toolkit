...
URL=`/opt/usi/usi get readycloud_fetch_url`

# auth data
NAS_NAME=`/opt/usi/usi get readycloud_hostname`
NAS_PASS=`/opt/usi/usi get readycloud_password`

#LOG_FILE="/tmp/readycloud_register.log"
	
# construct comm exec
CA_FILE=`/opt/usi/usi get readycloud_remote_certificate`
if [ -z $CA_FILE ]; then
    COMM_EXEC="curl --basic -k --user ${NAS_NAME}:${NAS_PASS} --url ${URL}"
else
    COMM_EXEC="curl --basic --tlsv1.2 --cacert ${CA_FILE} --user ${NAS_NAME}:${NAS_PASS} --url ${URL}"
fi
...
do_register()
{
	...
	USER_PASS=$2
	temp_dir=$3
	XAGENT_ID=$(/opt/usi/usi get x_agent_id | tr -d '[[:space:]]')
	...
	comm_post "${DATA}" && {
	   if [ "xSUCCESS" = "x${COMM_RESULT}" ]; then
		/opt/usi/usi set readycloud_registration_owner ${USER_NAME}
        /opt/usi/usi set leafp2p_run "1"
        #/opt/usi/usi set x_force_connection "1"
        /opt/usi/usi commit
        if [ -f /tmp/xagent_watchdog.pid ]; then
        	#kill -SIGHUP `cat /tmp/xagent_watchdog.pid`
		/opt/xagent/xagent_control -c on_readycloud -s rc -r true
        fi
...
do_unregister()
{
	...
	comm_post "${DATA}" && {
	if [ "xSUCCESS" = "x$COMM_RESULT" ]; then
            /opt/usi/usi unset readycloud_registration_owner
            /opt/usi/usi set leafp2p_run "0"
            #/opt/usi/usi unset x_force_connection
            /opt/usi/usi unset leafp2p_username
            /opt/usi/usi unset leafp2p_password
            /opt/usi/usi commit

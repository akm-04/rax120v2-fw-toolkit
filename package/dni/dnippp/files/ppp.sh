...
cp /tmp/resolv.conf /etc/ppp/ppp-dns.conf
...
qos_enable=\$(/bin/config get qos_endis_on)
qos_bandwidth_enable=\$(/bin/config get qos_threshold)
qos_bandwidth_type=\$(/bin/config get qos_bandwidth_type)
if [ "x\$qos_enable" = "x1" -a "x\$qos_bandwidth_enable" = "x1" ]; then
	if [ "x\$qos_bandwidth_type" = "x1" ]; then
		/etc/bandcheck/band-check &
	fi
fi
ipv6_wantype=\$(/bin/config get ipv6_type)
if [ "x\$ipv6_wantype" = "x6to4" ]; then # -a "\$ipv6_wantype" != "disabled" -a "\$ipv6_wantype" != "bridge" -a "\$ipv6_wantype" != "pppoe" ]; then    
	killall net6conf
	/etc/net6conf/net6conf restart
fi

if [ "x\$ipv6_wantype" != "xdisabled" ] && [ -s /tmp/ipv6_dns ]; then

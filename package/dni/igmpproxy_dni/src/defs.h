...
/*
 * to fix compile warning (to support 64 bit platform)
 * implicit declaration of function will return wrong pointer address
 */
#if (DNI_MULTI_LAN_SUPPORT)
...
#endif
#if (IGMP_IMMEDIATE_LEAVE)
extern struct Listener *findListener(struct RouteTable*  croute, uint32 src);
#endif
extern int mc_ctrl_packet(uint32 group);
extern int getMcGroupSock();
extern int inetChksum(u_short *addr, u_int len);
extern void fork_pcap_ppp();
extern void check_mbr_timeout(void *data);
extern void enable_bt_igmp(int group);
extern long GetSysUpTime();
extern int add_source_list(uint32 mgroup, uint32 src_ip);
extern int set_source_list(struct RouteTable *croute);
extern void processV3report(char *buf, uint32 src, uint32 port);
extern void update_all_ports(struct RouteTable *croute);

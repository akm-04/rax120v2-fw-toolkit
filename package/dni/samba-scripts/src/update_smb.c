...
#define USB_SMB_NAME	"NETGEAR RAX120"
...
static void load_share_info(FILE *fp, char *diskname)
{
	...
	*/
	...
	list_for_each(pos, &disk_lists) {
		...
		int j, i;
		int star_count = 0;
		char name[64], *val, oneline[1024], device_id[128], volume[128], value1[128]={0};
		...
		for(j=0; ;j++){
			...
			if (share_name == NULL ||folderName == NULL ||
					...
					continue;			
			if (!strcmp(device_id,diskinfo->device_id) && strcmp(volumeName,diskinfo->vol_name)) {
				snprintf(value1,sizeof(value1),"%s*/*0*0*%s*%s*%s*%s",share_name,diskinfo->vol_name,deviceName,serial_num,partition_id);
				config_set(name,value1);
			}
	...
	if (no_shareinfo && diskname != NULL) {
		...
		int i;
		//FILE *tmpfp;
		char value[256],name[64],*val,*sep,line[256],*serial_n,*part_id,disk_id[128]={0};
		...
		list_for_each(pos, &disk_lists) {
			...
			for(i=0;i<100 ;i++){
				...
				if(*val == '\0'){
					...
					strtok(NULL,sep); /*reader access*/
					strtok(NULL,sep); /*write access*/
